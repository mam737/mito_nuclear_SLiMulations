#!/bin/bash -l
#SBATCH --job-name=slim_nonWF_psm
...
cp psm_nonWF_mito_${myModel}.slim /workdir/$USER/$newdir
...
for idx in $(seq 1 10); do
  ...
  /programs/SLiM/slim -d N=${myNe} -d mig=${myMig} -d MC_R=${myMCR} -d MC_del=${myMCdel} -d MC_adv=${myMCadv} -d "filename1='${fn1}'" -d "filename2='${fn2}'" -d "filename3='${fn3}'" psm_nonWF_mito_${myModel}.slim
done

# copy out results
outdir=/fs/cbsuclarkfs1/storage/mam737/SLiMULATION/psm_nonWF/${myModel}/NE_${myNe}/mig_${myMig}/${myMCR}_${myMCdel}_${myMCadv}
