#!/bin/bash -l
#SBATCH --job-name=slim_nonWF_cssm
...
myMigf=`sed -n "${SLURM_ARRAY_TASK_ID}p" params.txt | cut -f 3`
myMigm=`sed -n "${SLURM_ARRAY_TASK_ID}p" params.txt | cut -f 4`
...
cp /home/mam737/SLiMulations/nonWF_cssm/run_10.6.20/cssm_nonWF_mito_${myModel}.slim /workdir/$USER/$newdir
...
for idx in $(seq 1 10); do
  ...
  /programs/SLiM/slim -d N=${myNe} -d f_mig=${myMigf} -d m_mig=${myMigm} -d MC_R=${myMCR} -d MC_del=${myMCdel} -d MC_adv=${myMCadv} -d "filename1='${fn1}'" -d "filename2='${fn2}'" -d "filename3='${fn3}'" cssm_nonWF_mito_${myModel}.slim
done

# copy out results
outdir=/fs/cbsuclarkfs1/storage/mam737/SLiMULATION/cssm_nonWF/${myModel}/NE_${myNe}/migf_${myMigf}_migm_${myMigm}/${myMCR}_${myMCdel}_${myMCadv}
